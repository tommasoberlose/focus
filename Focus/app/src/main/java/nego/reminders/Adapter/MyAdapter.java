...
import nego.reminders.SwipeDismissTouchListener;
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        if (mDataset.get(position).getType() == 8) {
            ...
            if (mDataset.get(position).getItem().getAlarm().equals(Costants.ALARM_TYPE_WHERE)) {
                if (mDataset.get(position).getItem().getDate_reminded() == 0)
                    subtitle.setText(mContext.getResources().getString(R.string.snoozed) + ": "+ LocationF.getPlace(mContext, mDataset.get(position).getItem().getAlarm_info()).getName());
                else if (mDataset.get(position).getItem().getDate_reminded() != 0)
                    subtitle.setText(mContext.getResources().getString(R.string.reminded) + ": "+LocationF.getPlace(mContext, mDataset.get(position).getItem().getAlarm_info()).getName());
            }
